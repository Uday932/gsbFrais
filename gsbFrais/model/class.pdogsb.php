 ...
 */
    ...
    */
    ...
    public function getInfosFraisMois($idVisiteur, $Unnmois,$idFraisForfait){
        $req = "SELECT idVisiteur, mois, idFraisForfait, (quantite * montant) as prix
                ...
                WHERE idVisiteur = '$idVisiteur' AND mois = '$Unnmois' AND idFraisForfait = '$idFraisForfait' ";
        $res = PdoGsb::$monPdo->query($req);
        $lesInfosFrais = array();
        ...
        return $lesInfosFrais;
    }
    public function getInfosFraisMois2($idVisiteur,$idFraisForfait){
        $req = "SELECT idVisiteur, mois, idFraisForfait, (quantite * montant) as prix
                FROM lignefraisforfait
                INNER JOIN fraisforfait ON lignefraisforfait.idFraisForfait = fraisforfait.id
                WHERE idVisiteur = '$idVisiteur'AND idFraisForfait = '$idFraisForfait' ";
        $res = PdoGsb::$monPdo->query($req);
        $lesInfosFrais = array();
    ...
    public function getVisiteur(){
        $req = "select distinct id from visiteur" ;
        $res = PdoGsb::$monPdo->query($req);
        $laLigne = $res->fetch();
        while($laLigne != null)	{
            $visiteur = $laLigne['id'];
    ...
    public function getCumulTous($idVisiteur,$Unmois){
        $req = "SELECT li.idVisiteur, li.idFraisForfait, li.mois,v.nom, v.prenom, li.quantite * ff.montant as somme from visiteur v 
        ...
        where v.id = '$idVisiteur' and fi.mois = '$Unmois'";
        $res = PdoGsb::$monPdo->query($req);
        $lesTypesFrais = array();
    ...
    public function getCumulToutVisiteur($idVisiteur){
        $req = "SELECT sum(quantite) * fraisforfait.montant as somme 
        FROM lignefraisforfait 
        inner join fraisforfait on idFraisForfait = id 
        where idVisiteur = '$idVisiteur' group by idFraisForfait;";
        $res = PdoGsb::$monPdo->query($req);
        $lesTypesFrais = array();
